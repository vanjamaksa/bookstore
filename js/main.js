...
if(url.indexOf("product.html")!=-1){
	...
	$('#genreDiv').click(sortFunction);
	$('#authorDiv').click(sortFunction);
	$('#statusDiv').click(sortFunction);
	...
	function filterByGenre(book) {
		...
		$("input:checkbox[name=genre]:checked").each(function() { choosenGenre.push(parseInt($(this).attr('value'))); });
		$("input:checkbox[name=author]:checked").each(function() { choosenAuthors.push($(this).attr('value')); });
		$("input:checkbox[name=status]:checked").each(function() { choosenStatuses.push($(this).attr('value')); });

		let filteredBooks = book;

		if (choosenGenre.length > 0) {
			filteredBooks = filteredBooks.filter((x) => choosenGenre.includes(x.category))
		}

		if (choosenAuthors.length > 0) {
			filteredBooks = filteredBooks.filter((x) => choosenAuthors.includes(x.author))
		}

		if (choosenStatuses.length > 0) {
			filteredBooks = filteredBooks.filter((x) => choosenStatuses.includes(x.status))
		}
	...
	function showBooks(book) {
							...
							<div class="product ftco-animate">
								...
								<div class="desc">
									<p class="meta-prod d-flex">
										<a class="addCart d-flex justify-content-center align-items-center" href="#" onclick="addToCart(${product.id})" ><span class="flaticon-shopping-bag"></span></a>
									</p>
								</div>
								</div>
		...
		if (book.length == 0)
		print += '<h3>Oops, we are unable to find a product matching your filters.</h3>';
...
function removeFromCart(productId) {
	...
	if (cart.length == 0)
	{
		...
		h2.innerHTML = 'Cart is currently empty';
...
function createCartItems() {
	ajaxCallback('books.json', (books) => {
		...
		cart.forEach((id) => booksInCart.push(books.find(x => id == x.id)));
		...
		booksInCart.forEach((book) => {
			...
			div.style.backgroundImage = `url(images/${book.img.src})`;
			...
			td = document.createElement('td');
			td.innerHTML = `${book.price.new ? book.price.new : book.price.old}$`;
